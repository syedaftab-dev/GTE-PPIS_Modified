...
def train(model, train_dataframe, valid_dataframe, fold = 0):
    train_loader = DataLoader(dataset=ProDataset(train_dataframe, fusion_mode=FUSION_MODE), batch_size=BATCH_SIZE, shuffle=True, num_workers=8, collate_fn=graph_collate)
    valid_loader = DataLoader(dataset=ProDataset(valid_dataframe, fusion_mode=FUSION_MODE), batch_size=BATCH_SIZE, shuffle=True, num_workers=8, collate_fn=graph_collate)
...
def train_full_model(all_dataframe, aver_epoch):
    ...
    train_loader = DataLoader(dataset=ProDataset(all_dataframe, fusion_mode=FUSION_MODE), batch_size=BATCH_SIZE, shuffle=True, num_workers=8, collate_fn=graph_collate)
